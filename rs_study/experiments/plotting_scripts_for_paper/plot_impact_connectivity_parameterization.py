...
def stripplot_mean_score(df, save_path, dic_model_dim, suffix=None, x=None,
    ...
    # measure labels
    if y == 'measure':
        # ylabel = "Connectivity Parameterization"
    ...
    axx.set_xlabel('Relative in prediction scores', fontsize=12,
                   fontweight='normal')
    if y == 'atlas':
        axx.xaxis.set_label_coords(0.58, -0.09) # (left, top)
    axx.set_ylabel('')
    # plt.text(.5, 1.02, ylabel, transform=axx.transAxes, size=15, ha='center')
    # xlabel = 'Relative in prediction scores'
    # plt.text(.01, 2.9, xlabel, size=12, ha='center', fontweight='normal')
    ...
    axx.legend([cobre_marker1, adni_marker2, adnidod_marker3, acpi_marker4, abide_marker5],
               ['COBRE', 'ADNI', 'ADNIDOD', 'ACPI', 'ABIDE'],
               loc=leg_pos, handletextpad=-0.55555, borderaxespad=0,
    ...
    plt.tight_layout(rect=[0, .1, 1, 1.], pad=0.1, w_pad=1)
...
dic_measure_n = {'partial correlation': 1,
                 'correlation': 2,
                 'tangent': 3}
###############################################################################
...
stripplot_mean_score(df, save_path, dic_model_dim, suffix=save_sufix, x=x,
                     ...
                     figsize=(4, 4), leg_pos='lower left', leg_borderpad=0.1,
                     leg_ncol=5, leg_bbox_to_anchor=(-0.31, -0.25))
                     #(-0.06, -0.04)
